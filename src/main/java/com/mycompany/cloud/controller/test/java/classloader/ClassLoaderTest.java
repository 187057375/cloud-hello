...
//        不知道大家有没有发现，不管是Bootstrap ClassLoader还是ExtClassLoader等，这些类加载器都只是加载指定的目录下的jar包或者资源。如果在某种情况下，我们需要动态加载一些东西呢？比如从D盘某个文件夹加载一个class文件，或者从网络上下载class主内容然后再进行加载，这样可以吗？
        ...
        loader.loadClass("com.mycompany.cloud.controller.test.java.classloader.DiskClassLoader");
        //使用Class.forName()来加载类，默认会执行初始化块
        // Class.forName("com.mycompany.cloud.controller.test.java.classloader.DiskClassLoader");
        //使用Class.forName()来加载类，并指定ClassLoader，初始化时不执行静态块
        Class.forName("com.mycompany.cloud.controller.test.java.classloader.DiskClassLoader", false, loader);
    }
